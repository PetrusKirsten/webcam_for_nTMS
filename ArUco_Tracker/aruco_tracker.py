        ...
        if np.all(ids is not None):
            if ids[0] == 0 or ids[0] == 1:  # If detect the probe markers
                ...
                if retval:
                    ...
                    DrawInfo.probe_arrow(frame, reprojectdst, reprojectdst_arrow)
                    DrawInfo.probe_coordinates(frame, euler_angle, tool_tip_position)
            ...
            if ids[0] == 2 or ids[0] == 3:  # If detect the coil markers
                ...
                if retval:
                    ...
                    aruco.drawDetectedMarkers(frame, corners)  # Draw a square around the markers
                    aruco.drawAxis(frame, cam_matrix, dist_coeffs, rvec, tvec, 0.1)
                    DrawInfo.coil_coordinates(frame, rvec, tvec)

        cv2.imshow("ArUco Tracker Test", frame)
